...
import io.github.nerd.discordkt.discord.events.ReactionAddEvent
...
/**
 * @author ashley
 * @since 6/1/17 12:35 AM
 */
internal class MessageReactionAddHandler : WebSocketResponseHandler<ReactionEventDataModel>("MESSAGE_REACTION_ADD", ReactionEventDataModel::class.java) {
		...
		if (discord.channels.has(data.channelID) && discord.users.has(data.userID)) {
			...
			if (chan is TextChannel && chan.messages.has(data.messageID)) {
				...
				if (data.emoji.id == null && discord.emojis.has(data.emoji.name)) {
					...
					discordLogger.debug("{} reacted to message {} with an unknown emoji", user.fullName, message.id)
					return
				}

				discordLogger.debug("{} reacted to message {} with emoji {}", user.fullName, message.id, emoji.names[0])
				val reaction = Reaction(message, emoji, user)
				message.reactions + reaction
				discord.events.dispatch(ReactionAddEvent(reaction))
			}
		}
	}
}